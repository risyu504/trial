# DOM インターフェイス データフォルダ readme

この `README.md` ファイルは、このフォルダに関しての説明を記述したファイルです。

## 概要

- このフォルダは、**DOM インターフェイス**の関連データファイルを収めるフォルダです。
...
4. このフォルダのフォルダ区分は、'**Hi**'(Htmlinterface) です。
...
- ※ データを分かり易くフォルダ分け
- `htmlinterface.json` 用意したデータファイルをスクリプトで纏めたデータファイル
...
- metadata: このコンテンツのコードリスト処理用データ。
  ...
  - anagindent: インデント(数値)
  - anagkbn: 区分
  - asagtype: コード種別("WebIDL")
- code: このコンテンツのデータ。(WebIDL)
  - exposed: 可用性
  - interface: インターフェイス
  - parent: 継承
  - includes: 含む物
