# 実行ファイルフォルダ readme

この `README.md` ファイルは、このフォルダに関しての説明を記述したファイルです。

## 概要

- このフォルダは、**プログラムの実行ファイル**を収めるフォルダです。(フォルダ構成は「[フォルダ構成・方針](../Document/00_Materials/folder_policy.md)」で記しています。)
- [開発フォルダ](../README.md)の下位フォルダとする事を、想定しています。
- ここでは主に gulp を用いるので、[Gulpfile](../Gulpfile/README.md)にその実行ファイルがあります。
- 名称: **Bin**, 区分: **Bn** です。

## 使い方

- 意味別にフォルダを用意します。
- 各ツールで実行します。実行方法は、ツールの使い方の方で調べて下さい。

## 内部規約

1. 主に [コーディングルール](../Document/20_Design/codingrules.md) に準じます。
2. 外部データを考慮して、ある程度の例外を認めます。ただ出来るならば上記のコーディングルールに合わせるように努力する事を求めます。尚、内部で作成したデータは規約に準じる必要があります。
3. このフォルダのフォルダ区分は、'**Bn**'(Bin) です。
4. このフォルダのファイルに関する規約は、以下の通りとする。
    1. このフォルダのファイル区分は、以下の通りとする。
        - 現在はありません。
    2. ファイル名は、外部データを考慮して自由とする。
    3. ファイルは必要に応じて、フォルダで分ける事とする。
5. 汎用的なフォルダ構成の為に用意しているだけで、現在は未使用です。故に実践的な規約はまだありません。

## ナビ

- [開発ホーム](../README.md)
  - **実行ファイル**
  - [出力先](../Dest/README.md)
  - [設定ファイル](../Config/README.md)
  - [データ](../Data/README.md)
  - [パッケージ](../Module/README.md)
  - [ソース](../Src/README.md)
  - [書類倉庫](../Document/README.md)

## 作成者

Copyright(C) 2025-2025 KOUTAN
